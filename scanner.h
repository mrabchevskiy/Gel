...
#if !defined(CoreAGI__Gel_COCO_SCANNER_H__)
#define CoreAGI__Gel_COCO_SCANNER_H__
...
public:
	...
	//Scanner( FILE* s );
	Scanner();
	void reset( const unsigned char* source, int len ); // NB
...
void Buffer::SetPos(int value) {
	...
	if ((value < 0) || (value > fileLen)) {
		...
		printf( "\n\n Scanner fatal error: buffer out of bounds access, position: %d\n", value );
		exit(1);
	}
...
void Scanner::reset( const unsigned char* source, int len ){
  assert( source );
	buffer = new Buffer( source, len );
	Init();
}
...
void Scanner::Init() {
	...
	start.set(91, 41);
	for (i = 43; i <= 43; ++i) start.set(i, 21);
	for (i = 45; i <= 45; ++i) start.set(i, 21);
	start.set(226, 22);
	start.set(194, 14);
	start.set(63, 25);
	start.set(96, 26);
	...
	*heapEnd = 0;
	...
	if (sizeof(Token) > COCO_HEAP_BLOCK_SIZE) {
		...
		printf( "\n\n Scanner fatal error: Too small COCO_HEAP_BLOCK_SIZE\n" );
		exit( EXIT_FAILURE );
	}
	...
	if (ch == 0xEF) { // check optional byte order mark for UTF-8
		...
		if (ch1 != 0xBB || ch2 != 0xBF) {
			...
			printf( "\n\n Scanner fatal error: Illegal byte order mark at start of file\n" );
			exit( EXIT_FAILURE );
		}
...
void Scanner::AppendVal(Token *t) {
	...
	if (((char*) heapTop + reqMem) >= (char*) heapEnd) {
		if (reqMem > COCO_HEAP_BLOCK_SIZE) {
			...
			printf( "\n\n Scanner fatal error: Too long token value\n" );
			exit( EXIT_FAILURE );
		}
		CreateHeapBlock();
	}
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 4:
			...
			else if (ch <= L'!' || (ch >= L'#' && ch <= 65535)) {AddCh(); goto case_4;}
			else {goto case_0;}
		case 5:
			case_5:
			{t->kind = 3; break;}
		case 6:
			case_6:
			if (ch == L'$' || (ch >= L'0' && ch <= L'9') || (ch >= L'@' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_6;}
		...
		case 22:
			if (ch == 136) {AddCh(); goto case_24;}
			else if (ch == 134) {AddCh(); goto case_17;}
			else {goto case_0;}
		case 23:
			case_23:
			recEnd = pos; recKind = 6;
		...
		case 24:
			case_24:
			if (ch == 133) {AddCh(); goto case_13;}
			else if (ch == 185) {AddCh(); goto case_16;}
			else if (ch == 152) {AddCh(); goto case_19;}
			else {goto case_0;}
		case 25:
			{t->kind = 12; break;}
		case 26:
